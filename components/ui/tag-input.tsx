    ...
    const handlePaste = React.useCallback(
      (e: React.ClipboardEvent<HTMLInputElement>) => {
        ...
        const newValues = e.clipboardData
          ...
          .map((text) => text.trim().replace(FORMATTING_REGEX, ""))
          .filter((text) => text.length > 0);

        const transformedValues = transformValue
          ? newValues.map((v) => transformValue(v))
          : newValues;
          ...
          case "Enter":
            ...
            if (inputValue) {
              const transformedValue = transformValue
                ...
                : inputValue;
              onValueChange([...value, transformedValue]);
              setInputValue("");
            }
            break;
        }
      },
      [activeIndex, value, inputValue, RemoveValue, transformValue]
    );
