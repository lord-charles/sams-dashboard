...
const INITIAL_VISIBLE_COLUMNS = ["firstName", "lastName", "learnerUniqueID", "reference",
    "gender",
    "state10",
    "county28",
    "payam28",
    ...
    const filteredItems = React.useMemo(() => {
        ...
        if (hasSearchFilter) {
            ...
            filteredstudents = filteredstudents.filter((user) => {
                ...
                const referenceMatch = user?.reference?.toLowerCase().includes(searchLower);
                const gradeMatch = user?.class?.toLowerCase().includes(searchLower);
                const uniqueIdMatch = user?.learnerUniqueID?.toString().includes(searchLower);

                return nameMatch || referenceMatch || uniqueIdMatch || gradeMatch;
            });
        }
            ...
            case "isPromoted":
                return (
                    <div className="flex flex-col">
                        <p className={`font-bold  capitalize ${user?.isPromoted ? 'text-green-500' : 'text-default-500'}`}>{user?.isPromoted ? 'Yes' : "No"}</p>
                    </div>
                );
            case "isDroppedOut":
                return (
            ...
            <div className="flex flex-col gap-4">
                <div className="flex justify-between gap-3 items-end">
                    <Input
                        ...
                        placeholder="Search by name | class | reference | unique id ...  "
                        size="sm"
...
const columns = [
    ...
    { name: "UNiQUEID", uid: "learnerUniqueID", sortable: true },
